...
fn main() {
    println!(">> Du's kinda meh payout calculator <<\n");
    loop {
        logic()
    }
}
...
fn logic() {
    ...
    let answer: f32 = dl / count * cpm * split; // Does the calculation

    let output = answer as i32; // Converts floating point to integer

    println!("The monthly payment comes out to: >> ${output} <<");
}
